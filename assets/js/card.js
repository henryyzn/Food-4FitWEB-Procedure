...
var CardValidator = {
    ...
    checkFlag: function(num) {
 	...
 	checkFullCard: function(num, cvv) {
		for (var flag in FLAGS) {
            if (FLAGS[flag] == FLAGS.default) {
                continue;
            }
            
			if (CardValidator.validateCard(FLAGS[flag], num)) {
                if ((FLAGS[flag].regexpCvv || FLAGS.default.regexpCvv).test(cvv)) {
                    return true;
                }
			}
		}
...
$(document).ready(function() {
    ...
    cardInstance.find("[data-card-number]").on("input", function() {
        ...
        cardInstance.find("[data-card-flag]").val(flag.flag || 0);
    });
